...
["arrow", "src/arrow.dart", , Y, {
Arrow: {"": "Object;position>,initPos,velocity,angle,width,height,lock",
  update$1: function(deltaTime) {
    var t1, target;
    if (this.lock) {
      t1 = this.velocity;
      target = new U.Vector(0, 0);
      target.x = t1.x;
      target.y = t1.y;
      this.angle = target.get$angle();
    }
    t1 = this.position;
    t1.x = t1.x + this.velocity.x * deltaTime;
    t1 = this.position;
    t1.y = t1.y + this.velocity.y * deltaTime;
  },
  reset$0: function(_) {
    var t1, t2;
    t1 = this.initPos;
    ...
    this.position = t2;
    this.lock = false;
    this.velocity = new U.Vector(0, 0);
  }
}}],
["", "arrow.dart", , V, {
main: function() {
  var renderer, input, t1, t2;
  renderer = new Z.Renderer(document.querySelector("#game"), null, null, window.innerWidth, window.innerHeight, null, document.querySelector(".assets .arrow"), document.querySelector(".assets .bg"), document.querySelector(".assets .dartLogo"));
  renderer._setupCanvas$0();
  input = K.Input$(renderer.canvas);
  t1 = window.innerWidth;
  t1.toString;
  t2 = window.innerHeight;
  t2.toString;
  new Q.Loop(Q.Game$(t1, t2, input), renderer).run$0();
}},
1],
...
_BroadcastSubscription: {"": "_ControllerSubscription;_eventState,_next@,_previous?,_controller,_onData,_onError,_onDone,_zone,_state,_cancelFuture,_pending",
...
ProgressElement: {"": "HtmlElement;position=", "%": "HTMLProgressElement"},
...
["game", "src/game.dart", , Q, {
Game: {"": "Object;width,height,gameTime,gravity,arrow,stopwatch,count,gameOver,clickTime,clickPosition,click,targetList",
  update$0: function() {
    var currentTime, t1, t2, angle, t3;
    currentTime = C.JSNumber_methods.$tdiv(this.stopwatch.get$elapsedTicks() * 1000, 1000000) / 100;
    t1 = this.gameTime;
    ...
    this._checkCollision$0();
    t2 = this.clickPosition;
    if (t2 != null) {
      angle = J.$sub$n(t2, this.arrow.position).get$angle();
      t2 = this.arrow;
      if (!t2.lock)
        t2.angle = angle;
    }
    ...
    if (t2.lock) {
      t3 = t2.velocity;
      t2.velocity = t3.$sub(t3, this.gravity);
    }
    this.arrow.update$1(currentTime - t1);
  },
  _shoot$1: function(time) {
    var speed, target, t1, t2;
    this.arrow.lock = true;
    speed = this.capSpeed$1(time);
    target = J.$sub$n(this.clickPosition, this.arrow.position);
    target.normalize$0();
    t1 = target.x;
    t2 = target.y;
  ...
  generateTargets$0: function() {
    ...
    for (t1 = this.targetList, i = 0; i < 5; ++i) {
      t2 = this.height;
      target = new S.Target(new U.Vector(this.width - height, t2 / 5 * i + t2 / 10), height, height, null, null, false, [document.querySelector(".assets .chrome"), document.querySelector(".assets .firefox"), document.querySelector(".assets .safari"), document.querySelector(".assets .opera"), document.querySelector(".assets .ie")]);
      target.img = target.images[i];
      J.set$width$x(target.img, C.JSNumber_methods.toInt$0(target.width));
      J.set$height$x(target.img, C.JSNumber_methods.toInt$0(target.height));
      target.defaultImg = document.querySelector(".assets .dart");
      t1.push(target);
    }
  },
  _checkCollision$0: function() {
    var t1, t2, arrowTop, t3;
    t1 = this.arrow;
    t2 = t1.position;
    arrowTop = new U.Vector(t2.x + t1.width / 2, t2.y);
    t1 = arrowTop.x;
    t2 = this.width;
    if (t1 > t2 - this.height / 10 && t1 < t2)
      H.IterableMixinWorkaround_forEach(this.targetList, new Q.Game__checkCollision_closure(this, arrowTop));
    t1 = this.arrow;
    t2 = t1.position;
    t3 = t2.x;
    if (t3 - t1.width / 2 > this.width || t2.y + t1.height / 2 > this.height || t3 < 0)
      t1.reset$0(t1);
  },
  Game$3: function(width, height, input) {
    var t1, arrowImg, t2, t3, t4;
    t1 = this.height;
    arrowImg = document.querySelector(".assets .arrow");
    t2 = J.getInterceptor$x(arrowImg);
    t3 = t2.get$height(arrowImg);
    t2 = t2.get$width(arrowImg);
    if (typeof t3 !== "number")
      throw t3.$div();
    if (typeof t2 !== "number")
      throw H.iae(t2);
    t4 = this.height;
    t4 = new Y.Arrow(new U.Vector(60, t1 / 2), new U.Vector(0, 0), new U.Vector(0, 0), 0, t4 / 10, t3 / t2 * t4 / 10, false);
    t2 = t4.position;
    t3 = new U.Vector(0, 0);
    t3.x = t2.x;
    t3.y = t2.y;
    t4.initPos = t3;
    this.arrow = t4;
    this.generateTargets$0();
    t4 = input._onDown;
    t3 = new P._BroadcastStream(t4);
    H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(t4, "_BroadcastStreamController", 0)]);
    t3.listen$1(new Q.Game_closure(this));
    t3 = input._onUp;
    t4 = new P._BroadcastStream(t3);
    H.setRuntimeTypeInfo(t4, [H.getRuntimeTypeArgument(t3, "_BroadcastStreamController", 0)]);
    t4.listen$1(new Q.Game_closure0(this));
    t4 = input._onMove;
    t3 = new P._BroadcastStream(t4);
    H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(t4, "_BroadcastStreamController", 0)]);
    t3.listen$1(new Q.Game_closure1(this));
  },
  static: {
Game$: function(width, height, input) {
  var t1, t2;
  t1 = new P.Stopwatch(null, null);
  t1.start$0(t1);
  t2 = P.List_List(null, S.Target);
  H.setRuntimeTypeInfo(t2, [S.Target]);
  t2 = new Q.Game(width, height, 0, new U.Vector(0, -4), null, t1, 0, false, 0, new U.Vector(0, 0), false, t2);
  t2.Game$3(width, height, input);
  return t2;
}}
...
Game_closure: {"": "Closure;this_0",
  call$1: function(data) {
    var t1 = this.this_0;
    t1.click = true;
    t1.clickPosition = data;
    t1.clickTime = C.JSNumber_methods.$tdiv(t1.stopwatch.get$elapsedTicks() * 1000, 1000000) / 100;
  },
  $is_args1: true
},
...
Game_closure0: {"": "Closure;this_1",
  call$1: function(data) {
    ...
    if (t1.click) {
      t1.clickPosition = data;
      t2 = C.JSNumber_methods.$tdiv(t1.stopwatch.get$elapsedTicks() * 1000, 1000000);
      t3 = t1.clickTime;
      t1.clickTime = 0;
      t1.click = false;
      t1._shoot$1(t2 / 100 - t3);
    }
  },
  $is_args1: true
},

Game_closure1: {"": "Closure;this_2",
  call$1: function(data) {
    this.this_2.clickPosition = data;
  },
  $is_args1: true
},

Game__checkCollision_closure: {"": "Closure;this_0,arrowTop_1",
  call$1: function(target) {
    var t1, t2, t3;
    if (!target.get$isHit()) {
      t1 = this.arrowTop_1.y;
      t2 = target.position.y;
      t3 = target.height / 2;
      if (t1 > t2 - t3 && t1 < t2 + t3) {
        target.img = target.defaultImg;
        target.isHit = true;
        t1 = this.this_0;
        t2 = t1.arrow;
        t2.reset$0(t2);
...
["input", "src/input.dart", , K, {
Input: {"": "Object;_onMove,_onDown,_onUp",
  Input$1: function(canvas) {
    var t1, t2;
    canvas.toString;
    t1 = new W._ElementEventStreamImpl(canvas, C.EventStreamProvider_mousedown._eventType, false);
    H.setRuntimeTypeInfo(t1, [null]);
    t2 = new W._EventStreamSubscription(0, t1._html$_target, t1._eventType, W._wrapZone(new K.Input_closure(this)), t1._useCapture);
    H.setRuntimeTypeInfo(t2, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
    t2._tryResume$0();
    t2 = new W._ElementEventStreamImpl(canvas, C.EventStreamProvider_mousemove._eventType, false);
    H.setRuntimeTypeInfo(t2, [null]);
    t1 = new W._EventStreamSubscription(0, t2._html$_target, t2._eventType, W._wrapZone(new K.Input_closure0(this)), t2._useCapture);
    H.setRuntimeTypeInfo(t1, [H.getRuntimeTypeArgument(t2, "_EventStream", 0)]);
    t1._tryResume$0();
    t1 = new W._ElementEventStreamImpl(canvas, C.EventStreamProvider_mouseup._eventType, false);
    H.setRuntimeTypeInfo(t1, [null]);
    t2 = new W._EventStreamSubscription(0, t1._html$_target, t1._eventType, W._wrapZone(new K.Input_closure1(this)), t1._useCapture);
    H.setRuntimeTypeInfo(t2, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
    t2._tryResume$0();
  },
  static: {
Input$: function(canvas) {
  var t1 = new K.Input(P.StreamController_StreamController$broadcast(null, null, false, null), P.StreamController_StreamController$broadcast(null, null, false, null), P.StreamController_StreamController$broadcast(null, null, false, null));
  t1.Input$1(canvas);
  return t1;
}}
...
Input_closure: {"": "Closure;this_0",
  call$1: function(evt) {
    var t1, t2, x, y;
    t1 = J.getInterceptor$x(evt);
    t2 = t1.get$offset(evt);
    x = t2.get$x(t2);
    x.toString;
    t1 = t1.get$offset(evt);
    y = t1.get$y(t1);
    y.toString;
    t1 = this.this_0._onDown;
    if (t1._state >= 4)
      H.throwExpression(t1._addEventError$0());
    t1._sendData$1(new U.Vector(x, y));
  },
  $is_args1: true
},

Input_closure0: {"": "Closure;this_1",
  call$1: function(evt) {
    var t1, t2, x, y;
    t1 = J.getInterceptor$x(evt);
    t2 = t1.get$offset(evt);
...
["", "src/loop.dart", , Q, {
Loop: {"": "Object;game,renderer",
  run$0: function() {
    this.game.update$0();
    this.renderer.draw$1(this.game);
    C.Window_methods.get$animationFrame(window).then$1(new Q.Loop_run_closure(this));
  }
},

Loop_run_closure: {"": "Closure;this_0",
  call$1: function(_) {
    return this.this_0.run$0();
  },
  $is_args1: true
}}],
["renderer", "src/renderer.dart", , Z, {
Renderer: {"": "Object;_container,canvas,_context,_width,_height,game,arrowImg,backgroundImg,dartImg",
  draw$1: function(game) {
    this.game = game;
    this._clear$0();
    this._renderBackground$0();
    this._renderArrow$0();
    this._renderTargets$0();
    this._renderScore$0();
  },
  _setupCanvas$0: function() {
    this.canvas = W.CanvasElement_CanvasElement(null, null);
    var t1 = J.get$children$x(this._container);
    t1.add$1(t1, this.canvas);
    J.set$width$x(this.canvas, this._width);
    J.set$height$x(this.canvas, this._height);
    this._context = J.getContext$1$x(this.canvas, "2d");
  },
  _clear$0: function() {
    var t1, t2, t3;
    t1 = this._context;
    J.setFillColorRgb$4$x(t1, 0, 0, 0, 1);
    t2 = this._width;
    t3 = this._height;
    t1.clearRect(0, 0, t2, t3);
    t1.fillRect(0, 0, t2, t3);
  },
  _renderBackground$0: function() {
    var t1, t2, t3, t4;
    t1 = this._context;
    t2 = this.backgroundImg;
    t3 = this._width;
    if (typeof t3 !== "number")
      throw t3.$div();
    t4 = J.get$width$x(t2);
    if (typeof t4 !== "number")
      throw t4.$div();
    J.drawImage$3$x(t1, t2, t3 / 2 - t4 / 2, 0);
    t4 = this._context;
    J.setFillColorRgb$4$x(t4, 0, 0, 0, 0.7);
    t4.fillRect(0, 0, t3, this._height);
  },
  _renderArrow$0: function() {
    var arrow, pos, t1, t2, t3;
    arrow = this.game.arrow;
    pos = arrow.position;
    t1 = this._context;
    J.save$0$x(t1);
    t1.translate(pos.x, pos.y);
    t1.rotate(arrow.angle);
    t2 = arrow.width;
    t3 = arrow.height;
    t1.drawImage(this.arrowImg, -t2 / 2, -t3 / 2, t2, t3);
    t1.restore();
  },
  _renderTargets$0: function() {
    H.IterableMixinWorkaround_forEach(this.game.targetList, new Z.Renderer__renderTargets_closure(this));
  },
  _renderScore$0: function() {
    var score, t1, t2, t3, t4, t5, t6;
    score = "Dart support: " + this.game.count * 20 + "%";
    t1 = this.game.gameOver;
    t2 = this._context;
    if (t1) {
      t1 = this.dartImg;
      t3 = this._width;
      ...
      if (typeof t4 !== "number")
        throw t4.$div();
      J.getInterceptor$x(t2).drawImage$3(t2, t1, t3 - t5 / 2, t6 - t4 / 2 - 50);
      t2.lineWidth = 2;
      t2.strokeStyle = "#0081c6";
      t2.fillStyle = "#00a4e4";
      t2.font = "60pt Arial";
      t3 -= 300;
      t6 += 200;
      t2.strokeText(score, t3, t6);
      C.CanvasRenderingContext2D_methods.fillText$3(t2, score, t3, t6);
    } else {
      J.getInterceptor$x(t2).set$lineWidth(t2, 2);
      t2.strokeStyle = "#0081c6";
      t2.fillStyle = "#00a4e4";
      t2.font = "20pt Arial";
      t2.strokeText(score, 10, 40);
      C.CanvasRenderingContext2D_methods.fillText$3(t2, score, 10, 40);
    }
  }
},

Renderer__renderTargets_closure: {"": "Closure;this_0",
  call$1: function(target) {
    var t1, pos, t2, t3, t4;
    t1 = J.getInterceptor$x(target);
    pos = t1.get$position(target);
    t2 = this.this_0._context;
    J.save$0$x(t2);
    t2.translate(J.get$x$x(pos), pos.y);
    t1 = t1.get$img(target);
    t3 = target.width;
    t4 = target.height;
    t2.drawImage(t1, -t3 / 2, -t4 / 2, t3, t4);
    t2.restore();
  },
  $is_args1: true
}}],
["target", "src/target.dart", , S, {
Target: {"": "Object;position>,width,height,img>,defaultImg,isHit<,images"}}],
["vector", "src/vector.dart", , U, {
Vector: {"": "Object;x>,y>",
  $eq: function(_, other) {
    var t1;
    if (other == null)
      return false;
    t1 = J.getInterceptor$x(other);
    return this.x === t1.get$x(other) && this.y === t1.get$y(other);
  },
  $sub: function(_, other) {
    return new U.Vector(this.x - other.x, this.y - other.y);
  },
  normalize$0: function() {
    var d = this.get$magnitude();
    if (d > 0) {
      this.x = this.x / d;
      this.y = this.y / d;
    }
...
S.Target.$isTarget = true;
S.Target.$isObject = true;
...
function dart_precompiled($collectedClasses) {
  ...
  function Arrow(position, initPos, velocity, angle, width, height, lock) {
  ...
  Arrow.prototype.get$position = function(receiver) {
  ...
  function Game(width, height, gameTime, gravity, arrow, stopwatch, count, gameOver, clickTime, clickPosition, click, targetList) {
    this.width = width;
    this.height = height;
  ...
  function Game_closure1(this_2) {
    this.this_2 = this_2;
  }
  Game_closure1.builtin$cls = "Game_closure1";
  if (!"name" in Game_closure1)
    Game_closure1.name = "Game_closure1";
  $desc = $collectedClasses.Game_closure1;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game_closure1.prototype = $desc;
  function Game__checkCollision_closure(this_0, arrowTop_1) {
    this.this_0 = this_0;
    this.arrowTop_1 = arrowTop_1;
  }
  Game__checkCollision_closure.builtin$cls = "Game__checkCollision_closure";
  if (!"name" in Game__checkCollision_closure)
    Game__checkCollision_closure.name = "Game__checkCollision_closure";
  $desc = $collectedClasses.Game__checkCollision_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game__checkCollision_closure.prototype = $desc;
  ...
  function Input(_onMove, _onDown, _onUp) {
  ...
  function Input_closure(this_0) {
    this.this_0 = this_0;
  }
  Input_closure.builtin$cls = "Input_closure";
  if (!"name" in Input_closure)
    Input_closure.name = "Input_closure";
  $desc = $collectedClasses.Input_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Input_closure.prototype = $desc;
  function Input_closure0(this_1) {
    this.this_1 = this_1;
  }
  Input_closure0.builtin$cls = "Input_closure0";
  if (!"name" in Input_closure0)
    Input_closure0.name = "Input_closure0";
  $desc = $collectedClasses.Input_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Input_closure0.prototype = $desc;
  function Input_closure1(this_2) {
    this.this_2 = this_2;
  }
  Input_closure1.builtin$cls = "Input_closure1";
  if (!"name" in Input_closure1)
    Input_closure1.name = "Input_closure1";
  $desc = $collectedClasses.Input_closure1;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Input_closure1.prototype = $desc;
  ...
  if ($desc instanceof Array)
    $desc = $desc[1];
  Loop.prototype = $desc;
  function Loop_run_closure(this_0) {
    this.this_0 = this_0;
  }
  Loop_run_closure.builtin$cls = "Loop_run_closure";
  if (!"name" in Loop_run_closure)
    Loop_run_closure.name = "Loop_run_closure";
  $desc = $collectedClasses.Loop_run_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Loop_run_closure.prototype = $desc;
  function Renderer(_container, canvas, _context, _width, _height, game, arrowImg, backgroundImg, dartImg) {
  ...
  function Renderer__renderTargets_closure(this_0) {
    this.this_0 = this_0;
  }
  Renderer__renderTargets_closure.builtin$cls = "Renderer__renderTargets_closure";
  if (!"name" in Renderer__renderTargets_closure)
    Renderer__renderTargets_closure.name = "Renderer__renderTargets_closure";
  $desc = $collectedClasses.Renderer__renderTargets_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Renderer__renderTargets_closure.prototype = $desc;
  function Target(position, width, height, img, defaultImg, isHit, images) {
    this.position = position;
    this.width = width;
    this.height = height;
    this.img = img;
  ...
  Target.prototype.get$position = function(receiver) {
    return this.position;
  };
  Target.prototype.get$img = function(receiver) {
    return this.img;
  };
  Target.prototype.get$isHit = function() {
    return this.isHit;
  };
  function Vector(x, y) {
    this.x = x;
